...
function CodeEditor() {
  ...
  return (
    <div className="grid grid-cols-6 h-[100vh]">
      ...
      <div
        className={`
  fixed top-0 left-0 w-[300px] h-screen bg-white z-10 transition-transform duration-300 ease-in-out
...
`}
      >
        {sideBar ? (
          <IoIosArrowBack
            ...
            size={30}
            className="absolute -right-10 top-1/2 transform -translate-y-1/2 bg-white rounded-r-md p-2 cursor-pointer"
          />
        ) : (
          <IoIosArrowForward
            onClick={() => setSidebar(!sideBar)}
        ...
        <div className="mt-4 flex gap-2 justify-center items-center">
          <img src={logo || "/placeholder.svg"} className="w-10 h-10" alt="" />
        ...
        <div className="flex flex-col gap-2 mt-4">
          {clients.map((client) => {
            return (
              <div key={client.socketId} className="flex gap-2 items-center bg-white px-6 py-2 rounded-lg">
                <div className="w-10 h-10 rounded-full bg-[#f5f5f5] flex justify-center items-center">
                  <img src={avatar || "/placeholder.svg"} className="w-8 h-8" alt="" />
                </div>
      ...
      {sideBar && <div className="fixed inset-0 bg-black bg-opacity-50 z-5" onClick={() => setSidebar(false)}></div>}
      <div
        className={`col-span-6 bg-[#f5f5f5] h-[100vh] ${sideBar ? "ml-[300px]" : ""} transition-margin duration-300`}
      >
        <TextEditor quill={quill} socket={socket} roomId={Id} onCodeChange={() => {}} targetRef={targetRef} />
      </div>
    </div>
  )
}
